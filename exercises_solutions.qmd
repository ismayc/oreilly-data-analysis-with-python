...
```{mermaid}
%%| fig-cap: "Either file type lands in the same DataFrame. Three calls then answer three questions about it."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}, 'flowchart': {'wrappingWidth': 500}}}%%
flowchart TD
  ...
  DF --> Q2["info()<br/>what type is each<br/>column, and what<br/>is missing?"]:::accent
...
```{mermaid}
%%| fig-cap: "Cleaning has an order. Settle what is missing, then types, then names, and cut down to the rows you want last."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}, 'flowchart': {'wrappingWidth': 500}}}%%
flowchart LR
  ...
  C --> E["4. Rows you want<br/>boolean filter"]:::proc
...
```{mermaid}
%%| fig-cap: "Two ways to handle a gap. Which one is right depends on whether you can spare the rows."
%%| fig-align: center
%%{init: {'theme': 'base', 'themeVariables': {'fontSize': '16px'}, 'flowchart': {'wrappingWidth': 500}}}%%
flowchart TD
...
- [ ] Inspect a DataFrame using `head()`, `info()`, and `describe()`
- [ ] Handle missing data by dropping rows or filling values with `dropna` and `fillna`
- [ ] Convert column data types and rename columns
...
## Walkthrough #5: Data Visualization Techniques with Matplotlib and Seaborn
...
flowchart TD
  ...
  Q -->|"how one number<br/>is spread out"| H["histplot<br/>bins plus a density curve"]:::proc
  Q -->|"the summary of one number<br/>inside each category"| B["boxplot<br/>median, quartiles, outliers"]:::proc
  Q -->|"the full shape of one number<br/>inside each category"| V["violinplot<br/>the whole distribution"]:::proc
  classDef decision fill:#fff3cd,stroke:#d39e00,color:#1a1a1a
  classDef proc fill:#dbe9ff,stroke:#1a73e8,color:#1a1a1a
```
...
## Exercise #5: Data Visualization Techniques with Matplotlib and Seaborn
...
- Draw a histogram, boxplot, and violin plot
...
plt.figure(figsize=(10, 6))
sns.boxplot(x='fertility_rate', y='continent', data=populations)
...
plt.figure(figsize=(10, 6))
sns.violinplot(x='fertility_rate', y='continent', data=populations)
plt.xlabel('Fertility Rate')
plt.ylabel('Continent')
...
## Walkthrough: Applying Your Skills, From Data to Insights
...
## Walkthrough: Finalizing and Presenting Your Data Analysis Project
